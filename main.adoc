...
* removal of the `sun.misc.Unsafe.defineClass` method. Users should use the public replacement,`java.lang.invoke.MethodHandles.Lookup.defineClass`, added in Java 9
* source incompatibility for `java.util.Stream.toArray(null)`, see https://bugs.openjdk.java.net/browse/JDK-8060192

For example, let's start from JEP 320. Several packages, such as JAXB, are not provided
by the JDK anymore so it is required to provide an external dependency for them.
...
jshell> List<Object> unmodificableList = Collections.unmodifiableList(list)
unmodificableList ==> []
...
==== Unmodificable collectors

Now it is possible to use `java.util.stream.Collectors` to build unmodificable collections, avoiding
