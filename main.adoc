...
The purpose is to allow receiver type to be annoted, later it is possible
...
Very specific yet useful method has been added to `java.util.Arrays`:
...
jshell> int[] triangularNumber(int n) {
   ...
   ...>   Arrays.setAll(result, idx -> idx + 1);
