...
As Java developers, we will busy in the upcoming months, upgrading our projects and products to
the newest Java releases. It will be a slow, incremental process, especially regarding the migration
from `classpath` to the `modulepath`.

This document tries to summarize new features in a very minimalistic style:
...
version. More details on the LTS releases can be found at https://blog.joda.org/2018/08/java-is-still-available-at-zero-cost.html.
