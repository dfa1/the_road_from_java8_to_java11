...
In practice <1> and <2> are merged into <3>.
...
@Deprecate(since="1.0", forRemoval=true)
...
In practice internal representation of string will be changed from char[] to byte[] + flag or
ISO-8859-1/Latin-1 (one byte per character) to UTF-16 (two bytes per character). This
should halve the memory required to represent constant strings, like database column names
or any other ascii string.

== Upgrade to Java8

The big deliver on Java8 was Project Lambda and java.time.
