...
As Java developers we will very busy in the following months upgrading our projects and products to
newest Java releases. It will be a slow, incremental process, especially regarding the upgrade from classpath
to the module path and new tooling. This document tries to summarize new features in a very minimalistic
style:

* focus is on the language level changes, secondarily on deploy/production
...
Java11 is the new LTS (Long Term Support), so it is über-important to upgrade as soon as possible to this
version of Java. More details on the LTS releases can be found at https://blog.joda.org/2018/08/java-is-still-available-at-zero-cost.html.
...
* official relese notes: https://jdk.java.net/11/release-notes
...
However, there are free, zero-cost, alteratives: https://jdk.java.net/11/[OpenJDK binaries] with related https://hub.docker.com/_/openjdk/[docker images]. Checkout also the https://adoptopenjdk.net/index.html[adoptopenjdk] project, that provides prebuilt JDK binaries as well as https://hub.docker.com/u/adoptopenjdk[docker images].
...
Several packages, such as JAXB, are not provided by the JDK anymore so it is required to provide an external dependency for them.
...
hello world!
----
<1> class name and file name must be match
...
Introduced in Java9, and promoted from incubator in Java11. This is a somewhat big API and it is
...
Everyhing looks fine, not let's write a simple HTTP2 client using `java.net.http` module:
...
Let's try simulating a busy web server, that delays each requests by 5 seconds, whereas our client timeouts after 1 second:
...
In this version of JDK it is possible to just use `String.isBlank`:
...
$2 ==> 0
----
<1> comparing a `StringBuilder` with `StringBuffer` yields 0 (they have the same content)
...
It is very convenient to use `java.nio.charset.StandardCharsets` (see https://docs.oracle.com/javase/7/docs/api/java/nio/charset/StandardCharsets.html[javadoc]):
...
Ofter using var is quite convenient:
...
Now it is possible to express things not possible before:
...
The small language changes included in Project Coin were low hanging fruits but was quite successfull.
JDK9 introduces the following small changes:
...
== Appendix: Upgrade to Java8

Since some projects and products are stuck in Java7 let's quickly cover JDK8.
